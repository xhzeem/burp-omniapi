...
This document maps Burp OmniAPI to PortSwigger's
...
| Criterion | OmniAPI evidence |
|---|---|
| Unique function | Provides authenticated, module-gated Montoya workflows, binary-safe message transport, live server controls, capability discovery, and generated OpenAPI documentation in one extension. |
| Clear name | Extension name is **Burp OmniAPI** and its suite tab is **OmniAPI**. |
| Secure operation | A fresh 256-bit API key protects every operational route; comparison is constant-time; CORS is disabled; JSON is strict; request bodies, registries, pagination, and regexes are bounded. Dangerous ShellUtils, shutdown, unload, options, persistence, and AI routes are excluded. |
| All dependencies | `burp-omniapi.jar` is a reproducible Shadow JAR containing Javalin, Jetty, Jackson, RE2/J, OpenAPI, and Swagger WebJar assets. Only the Burp-provided Montoya artifact is `compileOnly`. `verifyShadowJar` enforces this layout. |
| Responsive threading | Server startup, shutdown, and restart run on the dedicated `omniapi-server-lifecycle` executor. Requests run on Jetty workers. No target HTTP or other slow operation runs on Swing's EDT. |
| Clean unloading | `OmniApiUnloadHandler` implements `ExtensionUnloadingHandler`; it idempotently stops Javalin, closes extension WebSockets, clears registries, and shuts down the lifecycle executor. An automated test verifies that the port can immediately be rebound. |
...
| Burp AI | Not applicable. OmniAPI contains no AI integration. |

## Mandatory REST architecture

- The Javalin/Jetty listener is instantiated inside the extension and defaults to
  `127.0.0.1:31337`.
- Bind address and port can be changed explicitly in the OmniAPI tab and only take effect after a
  controlled restart.
- Operational endpoints require either the recommended `X-API-Key` header or the `apiKey` query
  ...
  `/health`, `/swagger`, `/openapi`, and the bundled Swagger UI assets under
...
The test suite covers authentication and key rotation, disabled modules, strict JSON,
