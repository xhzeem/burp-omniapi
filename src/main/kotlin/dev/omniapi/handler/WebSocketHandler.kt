...
import dev.omniapi.model.WebSocketConnectRequest
import dev.omniapi.model.WebSocketConnectResponse
import dev.omniapi.model.WebSocketEventDto
import dev.omniapi.model.WebSocketEventPage
import dev.omniapi.model.WebSocketSendRequest
...
class WebSocketHandler(private val api: MontoyaApi, private val gate: OperationGate) {
    ...
    fun connect(ctx: Context) = gate.run {
        if (sessions.size >= MAX_SESSIONS) {
            throw dev.omniapi.server.TooBusy("The WebSocket session registry is full")
        }
