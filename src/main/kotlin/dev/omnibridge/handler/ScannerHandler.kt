...
import dev.omnibridge.model.IssueDto
import dev.omnibridge.model.IssuePage
import dev.omnibridge.model.ScanRequest
import dev.omnibridge.model.ScanResponse
import dev.omnibridge.model.ScanTaskDto
...
class ScannerHandler(private val api: MontoyaApi, private val gate: OperationGate) {
    ...
    @OpenApi(
        ...
        responses = [
            ...
            OpenApiResponse(status = "400", content = [OpenApiContent(from = dev.omnibridge.model.ErrorResponse::class)]),
    ...
    fun scan(ctx: Context) = gate.run {
        if (tasks.size >= MAX_TASKS) {
            throw dev.omnibridge.server.TooBusy("The scanner task registry is full; restart OmniBridge to clear it")
        }
    ...
    @OpenApi(
        ...
        responses = [
            ...
            OpenApiResponse(status = "404", content = [OpenApiContent(from = dev.omnibridge.model.ErrorResponse::class)])
        ],
        security = [OpenApiSecurity(name = "ApiKeyAuth"), OpenApiSecurity(name = "ApiKeyQueryAuth")]
    )
