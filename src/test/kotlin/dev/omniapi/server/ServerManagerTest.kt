...
import dev.omniapi.OmniApiUnloadHandler
...
class ServerManagerTest {
    ...
    fun `unload handler releases listener port and lifecycle thread`() {
        ...
        val manager = ServerManager(api, state)
        val unloadHandler = OmniApiUnloadHandler(manager)
        ...
        eventually {
            Thread.getAllStackTraces().keys.none { it.isAlive && it.name == "omniapi-server-lifecycle" }
        }
    }
